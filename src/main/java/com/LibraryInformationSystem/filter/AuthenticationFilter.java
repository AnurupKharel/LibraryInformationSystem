	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		if ("admin".equals(userRole)) {
			...
			if (isLoggedIn) {
				...
				} else if (uri.endsWith(DASHBOARD) || uri.endsWith(LOGOUT)) {
					chain.doFilter(request, response);
				} else if (uri.endsWith(BROWSE) || uri.endsWith(CONTACT) || uri.endsWith(PROFILE) || uri.endsWith(ABOUT)
						|| uri.endsWith(HOME)) {
					res.sendRedirect(req.getContextPath() + DASHBOARD);
				} else {
					res.sendRedirect(req.getContextPath() + DASHBOARD);
			...
			if (isLoggedIn) {
				...
				} else if (uri.endsWith(HOME) || uri.endsWith(ABOUT) || uri.endsWith(PROFILE) || uri.endsWith(CONTACT)
						|| uri.endsWith(LOGOUT) || uri.endsWith(BROWSE)) {
					chain.doFilter(request, response);
